        ...
        while True:
            ...
            if "text" in msg:
                ...
                if t == "start":
                    ...
                    with contextlib_suppress(WebSocketDisconnect):
                        await ws.send_json({"type": "ack", "event": "start"})
                    print(f"[WS {cid}] start ack (json) sr={sample_rate}")
...
async def _finalize_turn(ws: WebSocket, audio_buf: io.BytesIO, sample_rate: int, cid: str):
    """
    PCM16 buffer -> WAV -> STT -> transcript -> LLM reply.
    Keeps socket open for next turns.
    """
...
async def _send_llm_reply(ws: WebSocket, prompt: str, cid: str):
    """
    Reliable path: non-streaming generate -> send once.
    Always: started -> delta -> assistant_final -> done
    """
    with contextlib_suppress(WebSocketDisconnect):
    ...
    # Safe logging (no backslashes in f-string expressions)
