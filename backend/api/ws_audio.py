        ...
        while True:
            ...
            if "text" in msg:
                ...
                if data.get("event") == "end":
                    ...
                    # Update long-term history & stream final answer
                    if final_text:
                        history.append({"role": "user", "content": final_text})
                        await start_llm_stream(final_text, tag="final")

                    # Close after LLM finishes (optional: or keep open)
                    # Wait a moment for tokens to flush (non-blocking if done)
            ...
            if chunk_count % PARTIAL_EVERY == 0:
                ...
                if text_clean and text_clean != last_partial:
                    ...
                    # Light debounce: only kick LLM if partial ends with space/punct or length grew > 8
